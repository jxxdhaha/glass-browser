          ...
          <section className="search-stage">
            ...
            {!submittedQuery ? (
              ...
              <div className="results-block reveal">
                ...
                <div className="results-list">{visibleResults.map((result, index) => <article className="result-card" key={result.title} style={{ animationDelay: `${index * 70}ms` }}><div className={`result-glow ${result.accent}`} /><div className="result-top"><span className="result-tag">{result.tag}</span><span className="result-time">{result.time}</span></div><h3>{result.title}</h3><div className="result-url">{result.url} <ExternalLink size={13} /></div><p>{result.description}</p><div className="result-footer"><button onClick={() => action(`Opening ${result.title}`)}>Open reference <ArrowRight size={14} /></button><button className="copy-button" aria-label="Copy result link" onClick={() => { navigator.clipboard?.writeText(result.url); toast("Link copied"); }}><Copy size={14} /></button></div></article>)}</div>
